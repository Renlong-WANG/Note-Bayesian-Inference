...
\]
denote the \emph{state} vector at time $t$, which represents the unknow quantity we want to infer.

At each time $t$, we have \emph{observation}
\[
...
Similarly, suppose that a \emph{control input} is available at each time $t$, denoted by
\[
...
Since subsequent inference depends on the state, the objective is to characterize the uncertainty associated with the random variable $\bm{x}_t$ given all available information. This uncertainty is represented by the \emph{belief state}
\[
