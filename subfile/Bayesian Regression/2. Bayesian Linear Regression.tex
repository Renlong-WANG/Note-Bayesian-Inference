...
In Bayesian regression, the coefficient vector $\bm{w}$ is treated as a random variable and assigned a prior distribution $p(\bm w)$. We assume that the prior distribution of $\bm{w}$ does not depend on $\mathbf{X}$, i.e., $w\perp X$, and thus $p(\bm w\mid\mathbf X)=p(\bm w)$.
